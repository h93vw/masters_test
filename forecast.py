...
def forecast_ARIMAX_fixed_order(meter_id, endog, observed, exog, observed_exog, pdq=(0, 1, 2)):
    ...
    try:
        ...
        # print('ARIMA{} - AIC:{}'.format(param, results.aic))
        individual_end_time = t.time()
        time = individual_end_time - individual_start_time
    ...
    program_end_time = t.time()
    print("fin-ARIMAX_forecast; Time: %d" % (program_end_time - program_start_time))
    return meter_id, mape, time


def forecast_ARIMAX(meterids, orders):
    times = []
    mapes = []
