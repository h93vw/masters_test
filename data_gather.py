...
def gather_cer_data_inc_init(current_time, number_of_meters=1, start=1000, window_size=datetime.timedelta(weeks=12), exog_check=False, forecast_check=False):
    ...
    for meter_id in meter_ids:
        ...
        query = "SELECT date, measurement FROM readings WHERE meterid = %d AND DATE < %s ORDER BY date" % meter_id, current_time
